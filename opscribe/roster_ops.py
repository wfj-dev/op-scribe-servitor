...
async def _collect_role_integrity_findings(guild: discord.Guild) -> list[dict]:
    ...
    blademaster = _role_name("blademaster", "Blade Master")
...
_AWARD_DISPATCH_FN_MAP = {
    ...
    "distinguished_kadaku_campaign_medal": "_get_distinguished_kadaku_campaign_announcement",
    ...
    "octavian_operation_medal": "_get_octavian_operation_announcement",
    "distinguished_octavian_operation_medal": "_get_distinguished_octavian_operation_announcement",
...
_CHALLENGE_AWARD_ROLE_MAP: dict[str, tuple[int, str]] = {
    ...
    "distinguished_kadaku_campaign_medal":     (DISTINGUISHED_KADAKU_CAMPAIGN_MEDAL_ROLE_ID,     "distinguished_kadaku"),
    ...
    "octavian_operation_medal":                (OCTAVIAN_OPERATION_MEDAL_ROLE_ID,                 "octavian_incident"),
    "distinguished_octavian_operation_medal":  (DISTINGUISHED_OCTAVIAN_OPERATION_MEDAL_ROLE_ID,   "distinguished_octavian_incident"),
...
async def chapter_request(
    ...
    if not requested_name or not _normalize_chapter_request_name(requested_name):
        await interaction.response.send_message(
            "You must provide a chapter name for `/chapter_request`.",
            ephemeral=True,
        )
        return
        ...
        if role_idx is not None:
            ...
            try:
                spec_map = {
                    ...
                    "kill team champion": "Blade Master",
                    "company champion": "Blade Master",
                    # Blade Master is a head role; mapping to itself is unnecessary
                }
                        ...
                        for row in rows_to_show:
                            ...
                            if member_id:
                                try:
                                    ...
                                    member_obj = None
                            display_name = (
                                str(getattr(member_obj, "display_name", "") or "")
                                if member_obj is not None
                                else str(row.get("member_label", row.get("member_id", "Unknown")))
                            )
                            mention_label = f"<@{member_id}>" if member_id else (display_name or "Unknown")
...
CHAMPION_ROLES = {"Bladeguard", "First Blade", "Blade Master"}
SPECIALIST_ROLES = {
    "Watch Chaplain",
    "Watch Apothecary",
    "Watch Librarian",
